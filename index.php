...
if ($id === 'referral') {
	...
	$player = mysql_fetch_assoc(mysql_query("SELECT * FROM zaidejai WHERE ip='$ip' LIMIT 1"));
	if (!$player) {
		header('Content-Type: application/json');
		$data = [
	...
	$euro = 3000;
	mysql_query("UPDATE zaidejai SET sms_litai=sms_litai+$euro WHERE nick='$player[nick]'");
	...
	mysql_query("INSERT INTO pm SET gavejas='$player[nick]', what='SISTEMA', txt='$playerMessage', time='" . time() . "', nauj='NEW'")or die(mysql_error());
...
if ($ip === '178.16.43.65') {
	...
	$b_laikas2 = time()+60;
	mysql_query("INSERT INTO ban_logai SET nick='testas1', uz='$message', time='$b_laikas2', kas_ban='SISTEMA'")or die(mysql_error);
	mysql_query("INSERT INTO block SET nick='testas1', uz='$message', time='$b_laikas2', kas_ban='SISTEMA'");
	...
	$insert1 = mysql_query("INSERT INTO pokalbiai SET nick='SISTEMA', sms='$priminimas', data='".time()."', expired_at='$expiresAt'");
	echo 'Zaidimas bus sustabdytas';
	die();
}


$ID  = isset($_GET[ID]) ? trim(mysql_real_escape_string(htmlspecialchars($_GET[ID]))) : null;
...
$kiek = mysql_num_rows(mysql_query("SELECT * FROM news "));
$reklama = mysql_fetch_assoc(mysql_query("SELECT * FROM sms_reklama ORDER BY id DESC LIMIT 1"));
...
$ref = mysql_real_escape_string($_GET['f']);
$refip = $_SERVER['REMOTE_ADDR'];
if(!empty($ref)){
	if(mysql_num_rows(mysql_query("SELECT * FROM zaidejai WHERE nick='$ref'")) < 1){
	}
	elseif(mysql_num_rows(mysql_query("SELECT * FROM referal WHERE nick='$ref' AND ip='$refip'")) == true){
		...
		mysql_query("INSERT INTO referal SET nick='$ref', ip='$refip'")or die(mysql_error());
		mysql_query("UPDATE zaidejai SET kred=kred+'10',atvede=atvede+'1', pts=pts+'0.1' WHERE nick='$ref'")or die(mysql_error());
		mysql_query("UPDATE atvedimas SET snd=snd+'1' WHERE nick='$ref'")or die(mysql_error());
...
<div class="up"><small><b><a href="?id=news"><font color="white">Paskutinis atnaujinimas ['.kiek('news').'+</font><font color="red"><b>'.$nust['sndnew'].'</b></font><font color="white">]</font></b></small></a></div></div>
 ...
 &bdquo;<font color="red"><b>'.smile($new['name']).'</font><small>&ldquo;['.laikas($new['data']).'] </small>
	...
	echo '</div>';
	$systemMessage = mysql_fetch_assoc(mysql_query("SELECT * FROM pokalbiai WHERE nick='SISTEMA' ORDER BY id DESC LIMIT 1"));
...
';
	...
	echo'</small><br/></span></div></div><div class="title1"><div style="text-align:left;vertical-align:middle;font-size:12px;">Žaidimo rodikliai<span style="float:right;"><a href="prisijungti.php?id=forget"/>[Pamiršau slaptažodį]</a></div></div><div class="content"><div style="text-align:left;vertical-align:middle;font-size:12px;">';
        ...
        $topUsers = mysql_query("SELECT * from zaidejai WHERE statusas != 'Kurejas' ORDER BY lygis DESC LIMIT 3");
        $count = 1;
...
<span style="text-shadow: 0px 0px 10px; float:right;"><small>Naujausias narys: '.($nust['new']).'</font></div></div>';
	...
	$query = mysql_query("SELECT * FROM reklama ORDER BY id DESC LIMIT 0,3");
