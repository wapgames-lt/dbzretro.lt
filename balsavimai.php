...
baneris();
		 topbar();
$kiekis = mysql_result(mysql_query("SELECT COUNT(*) FROM b_komentarai"), 0);
...
if($id == ''){
  ...
  $nr = abs((int)$_GET['nr']);
	$inf = mysql_fetch_assoc(mysql_query("SELECT * FROM bals WHERE id='1'"));
	echo'
	<div class="meniuc">
	...
	<b>1.</b> '.$inf['ats'].': '.mysql_num_rows(mysql_query("SELECT * FROM b_rez WHERE ats='1' && bals_id='1'")).' </br>
	<b>2.</b> '.$inf['ats2'].': '.mysql_num_rows(mysql_query("SELECT * FROM b_rez WHERE ats='2' && bals_id='1'")).'<br>
	<b>3.</b> '.$inf['ats3'].': '.mysql_num_rows(mysql_query("SELECT * FROM b_rez WHERE ats='3' && bals_id='1'")).'<br>
...
if($id == 'bals'){
	...
	if(mysql_num_rows(mysql_query("SELECT * FROM b_rez WHERE nick='$nick' && bals_id='1'")))
	{
		echo'<div class="meniuc"> Čia jau balsavai.</div>';
	 $g_n[] = array("pagrindinis.php?id=","Pagrindinis","balsavimai.php", "Balsavimas","Blasuoju");
	navigacija($g_n);
	}
	elseif(!mysql_num_rows(mysql_query("SELECT * FROM bals WHERE id='1'")))
	{
	...
	navigacija($g_n);
		mysql_query("INSERT INTO b_rez SET nick='$nick',ats='$kl',bals_id='1'");
...
elseif($id == "komentarai2"){
 ...
 echo '<div class="meniuc"><b>Klaida!</b> Tu esi užtildytas!</div>';
			...
			mysql_query("INSERT INTO `b_komentarai` (`nick`, `komentaras`, `laikas`) VALUES ('$nick', '$kom', '".time()."')") or die(mysql_error());
		}
	  $g_n[] = array("pagrindinis.php?id=","Pagrindinis","balsavimai.php", "Balsavimas", "Komentarai");
	navigacija($g_n);
	}
