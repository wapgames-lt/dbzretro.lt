...
if ($id == "") {
    ...
    top("Mano menių");
    if ($apie['kovuimg'] == "-") {
        $kovuimg = "įjungti kovų paveiksliukus";
    } else {
        $kovuimg = "Išjungti kovų paveiksliukus";
    }
    echo '
          <div class="meniuc">
...
' . $ico . ' <a href="?id=mod2&ka=uztildyti">Užtildyti žaidėją</a><br />        
  ' . $ico . ' <a href="?id=mod2&ka=unmute">Nuimti užtildymą</a><br />        
    ...
    if ($statusas == "Kurejas" or $apie['nick'] == 'testas1') {
        if ($nust['reg'] == "-") {
            $kkk = "įjungti registraciją";
        } else {
            $kkk = "Išjungti registracija";
        }
        if ($nust['kovos'] == "-") {
            $kkk2 = "įjungti kovų lauką";
        } else {
            $kkk2 = "Išjungti kovų lauką";
        }
        if ($nust['pas'] == "-") {
            $kkk3 = "įjungti pasiūlymų rašymą";
        } else {
            $kkk3 = "Išjungti pasiūlymų rašymą";
        }
        if ($nust['topic'] == "-") {
            $kkk4 = "įjungti topic";
        } else {
        ...
        if ($nust['pasiekimai'] == "-") {
            $kkk5 = "Įjungti pasiekimus";
        } else {
            $kkk5 = "Išjungti pasiekimus";
        }
        if ($nust['misijos'] == "-") {
            $kkk6 = "Įjungti misijas";
        } else {
            $kkk6 = "Išjungti misijas";
        }
    ...
    if ($statusas != "Mod" && $statusas != "Mod2" && $statusas != "Admin" && $statusas != "Mod3" && $statusas != "vmod" && $statusas != 'Mod4' && $statusas != 'Kurejas') {
        ...
        if ($ka == "ban_log") {
            ...
            if ($viso > 0) {
                ...
                echo '</div>';
                echo '<div class="meniuc">' . puslapiavimas($puslapiu, $psl, '?id=mod&ka=ban_log') . '</div>';
            ...
            if (isset($_POST['submit'])) {
                $b_nick = post($_POST['nick']);
                $b_p = post($_POST['przt']);
                $b_laikas = post($_POST['laikas']);
        ...
        <input type="submit" name="submit" value="OK"></form>
            ...
            if (mysqli_num_rows(mysqli_query($conn, "SELECT * FROM foto WHERE ar_patvirtinta='ne' AND id='$ft_id'")) == true) {
                ...
                if ($leid == 'taip') {
                    ...
                    if (empty($kom)) {
                        echo '<div class="meniuc">Komentaras būtinas</div>';
                    } else {
    ...
    if ($statusas != "Mod2" && $statusas != "Mod3" && $statusas != "Admin" && $statusas != "Mod4" && $statusas != "Kurejas") {
        ...
        if ($ka == "bals") {
            ...
            if (isset($_POST['submit'])) {
                $kl = post($_POST['klsm']);
                $ats = post($_POST['ats1']);
                $ats2 = post($_POST['ats2']);
                $ats3 = post($_POST['ats3']);
...
<br/><select name=\"nuot\">
";
    $slines = file("txt/nuotaikos.txt");
    $smax = count($slines);
    $i = "0";
    do {
        $dts = explode("|", $slines[$i]);
...
if ($id == 'nustatymai') {
    ...
    if ($apie['minichatatas'] == 1) {
        echo '<option value="1" selected="selected">Išmanusis</option>';
    } else {
        echo '<option value="1">Išmanusis</option>';
    }
    if ($apie['minichatas'] == 0) {
        echo '<option value="0" selected="selected">Paprastas</option>';
    } else {
        echo '<option value="0">Paprastas</option>';
    }
    echo '   </select><br/>
    ...
    if ($apie['mini_chat'] == 1) {
        echo '<option value="1" selected="selected">Ijungtas</option>';
    } else {
        echo '<option value="1">Ijungtas</option>';
    }
    if ($apie['mini_chat'] == 0) {
        echo '<option value="0" selected="selected">Išjungtas</option>';
    } else {
        echo '<option value="0">Išjungtas</option>';
    }
    echo '
        ...
        Pranešti apie turnyro pradžia?:<select name="trn">
           ';
    if ($user['rodyti_turnyra'] == 0) {
        echo '<option value="0" selected="selected">Išjungtas</option>';
    } else {
        echo '<option value="0">Išjuntas</option>';
    }
    if ($user['rodyti_turnyra'] == '1') {
        echo '<option value="1" selected="selected">Ijungtas</option>';
    } else {
        echo '<option value="1">Ijungtas</option>';
    }
...
if ($id == 'greitasiskeitimas') {
    ...
    if ($user['greitas'] == 'Pradžia') {
        echo '<option text="Pradžia" selected="selected">Pradžia</option>';
    } else {
        echo '<option text="Pradžia">Pradžia</option>';
    }
    if ($user['greitas'] == 'Kovu zona') {
        echo '<option text="Kovu zona" selected="selected">Kovu zona</option>';
    } else {
        echo '<option text="Kovu zona">Kovu zona</option>';
    }
    ...
    if ($user['greitas'] == 'Miestas') {
        echo '<option text="Miestas" selected="selected">Miestas</option>';
    } else {
        echo '<option text="Miestas">Miestas</option>';
    }
    if ($user['greitas'] == 'Bosai') {
        echo '<option text="Bosai" selected="selected">Bosai</option>';
    } else {
        echo '<option text="Bosai">Bosai</option>';
    }
    if ($user['greitas'] == 'Inventorius') {
        echo '<option text="Inventorius" selected="selected">Inventorius</option>';
    } else {
        echo '<option text="Inventorius">Inventorius</option>';
    }
    if ($user['greitas'] == 'Apie mane') {
        echo '<option text="Apie mane" selected="selected">Apie mane</option>';
    } else {
        echo '<option text="Apie mane">Apie mane</option>';
    }
    if ($user['greitas'] == 'Misijos') {
        echo '<option text="Misijos" selected="selected">Misijos</option>';
    } else {
        echo '<option text="Misijos">Misijos</option>';
    }
    ...
    if ($user['greitas'] == 'Pasiekimai') {
        echo '<option text="Pasiekimai" selected="selected">Pasiekimai</option>';
    } else {
        echo '<option text="Pasiekimai">Pasiekimai</option>';
    }
    if ($user['greitas'] == 'Meniu') {
        echo '<option text="Meniu" selected="selected">Meniu</option>';
    } else {
        echo '<option text="Meniu">Meniu</option>';
    }
    if ($user['greitas'] == 'Mano skill') {
        echo '<option text="Mano skill" selected="selected">Mano skill</option>';
    } else {
        echo '<option text="Mano skill">Mano skill</option>';
    }
    if ($user['greitas'] == 'PM dezute') {
        echo '<option text="PM dezute" selected="selected">PM dezute</option>';
    } else {
        echo '<option text="PM dezute">PM dezute</option>';
    }
    if ($user['greitas'] == 'Eurai') {
        echo '<option text="Eurai" selected="selected">Eurai</option>';
    } else {
        echo '<option text="Eurai">Eurai</option>';
    }
        ...
        if (isset($_POST['submit'])) {
            ...
            if (empty($kam) or empty($kaa) or empty($kiekis)) {
                ...
                if ($kaa == 70) {
                    ...
                    mysqli_query($conn, "INSERT INTO pm (what, txt, `time`, nauj, gavejas) VALUES ('SISTEMA', '$txt', '" . time() . "', 'NEW', '$kam')");
