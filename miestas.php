...
baneris();
topbar();
$a1 = mysql_result(mysql_query("SELECT COUNT(*) FROM aukcijonas"), 0);
$a2 = mysql_result(mysql_query("SELECT COUNT(*) FROM turgus"), 0);
$a3 = mysql_result(mysql_query("SELECT COUNT(*) FROM uzsakymai"), 0);
$a4 = mysql_result(mysql_query("SELECT COUNT(*) FROM nick_turgus"), 0);
if($id == ""){
	 online('Miestas');
   top('Miestas');
...
if($id == 'lenta'){
	...
	echo'<div class="meniu">';
			$viso = mysql_result(mysql_query("SELECT COUNT(*) FROM zaidejai WHERE rep_teig !='0'"),0);
		if($viso > 0){
			$rezultatu_rodymas=10;
            $total = @intval(($viso-1) / $rezultatu_rodymas) + 1;
            if (empty($psl) or $psl < 0) $psl = 1;
			...
			$puslapiu=ceil($viso/$rezultatu_rodymas);
		$query = mysql_query("SELECT * FROM zaidejai ORDER BY rep_teig DESC LIMIT $nuo_kiek,$rezultatu_rodymas");
...
top('Raktai');
        ...
        if($inv['red_key']< 7 or $inv['blue_key'] < 7 or $inv['yellow_key'] < 7 or $inv['black_key'] < 7 or $inv['green_key'] < 7){
            ...
            mysql_query("UPDATE zaidejai SET litai=litai+'$ev_zen', kred=kred+'$ev_krd', sms_litai=sms_litai+'$ev_lt' WHERE nick='$nick' ");
        mysql_query("UPDATE inv SET blue_key=blue_key-'7', yellow_key=yellow_key-'7', black_key=black_key-'7', red_key=red_key-'7', green_key=green_key-'7' WHERE nick='$nick'");
        }
     $g_n[] = array("pagrindinis.php?id=","Pagrindinis","miestas.php","Miestas", "Raktų keitykla");
	navigacija($g_n);
...
echo '<div class="meniuc"><img src=img/gydymo.jpeg border="1" width="100" height="50"><alt="**"></div>';
	...
	echo '<div class="meniuc"><img src=img/gydymo.jpeg border="1" width="100" height="50"><alt="**"></div>';
      ...
      mysql_query("UPDATE zaidejai SET gyvybes='$apie[max_gyvybes]', kred=kred-'500' WHERE nick='$nick'");
	  $ti = time()+0;
	    mysql_query("INSERT INTO block SET nick = '$nick', uz='gydosi kapsuleja', kas_ban='SISTEMA', time='$ti'");
  ...
  top('Pavasario eventas');
    ...
    online('Pavasario eventas');
      ...
      if($inv['zaislas1']< 10 or $inv['zaislas2'] < 10 or $inv['zaislas3'] < 10 or $inv['zaislas4'] < 10 ){
        ...
        mysql_query("UPDATE inv SET zaislas1=zaislas1-'10', zaislas2=zaislas2-'10', zaislas3=zaislas3-'10', zaislas4=zaislas4-'10' WHERE nick='$nick'");
       }
