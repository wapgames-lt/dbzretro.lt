...
if($id == ""){
	...
	if(mysql_num_rows(mysql_query("SELECT * FROM nick_turgus")) == false){
	echo'<div class="meniuc"><b>Parduodamų nick nėra</b></div>';
	}
	else
	{
	  $viso = mysql_result(mysql_query("SELECT COUNT(*) FROM nick_turgus"),0);
   if($viso > 0){
       $rezultatu_rodymas=5;
       ...
       $query = mysql_query("SELECT * FROM nick_turgus ORDER BY id DESC LIMIT $nuo_kiek,$rezultatu_rodymas");
