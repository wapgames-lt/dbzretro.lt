...
$ID = sk($_GET['ID']);
   online('Kasykloje');
   $lok = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM kasykla2 WHERE id='$ID' "));
echo'<div class="meniuc"><img src="img/kasimas/kasykla.png" border="1"></div>
...
echo'<div class="meniuc"><a href="?id=rudos"><font color="red"><b>Iškasti ištekliai</b></font></a></div>';
    if(mysqli_num_rows(mysqli_query($conn,"SELECT * FROM kasykla2 WHERE id='$ID' ")) == 0){
          echo '<div class="up"><b>Klaida!</b></div>';
          echo '<div class="meniuc">Tokios vietos nėra!</div>';
    } else {
        $total = mysqli_fetch_row(mysqli_query($conn, "SELECT COUNT(*) FROM kasyklav2 WHERE lokacija='$ID'"))[0];
...
echo '<div class="up">'.$ico.'Jūsų (<b>'.$apie['kasimolvl'].'   LVL</b> kasimo)</div>';
             echo '';
             $query = mysqli_query($conn,"SELECT * FROM kasyklav2 WHERE lokacija='$ID' ");
             while($row = mysqli_fetch_assoc($query)){
...
$kasimoreward=0;}
   $lok = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM kasykla2 WHERE id='$ID' "));
   $mob = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM kasyklav2 WHERE id='$VS' "));
   $m = mysqli_fetch_array(mysqli_query($conn,"SELECT * FROM zaidejai WHERE nick='$nick'"));
    ...
    if(mysqli_num_rows(mysqli_query($conn,"SELECT * FROM kasyklav2 WHERE id='$VS' ")) == 0){
          echo '<div class="up">Klaida !</div>';
